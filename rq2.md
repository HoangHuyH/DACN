...
class CTIPoisoningEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        self._write_to_log(f"\n--- Reset Env: Phase={self.poison_phase}, Seed={seed} ---")
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        self.current_turn += 1
        variant_id = int(action[0])
        channel_id = int(action[1])
        frequency = int(action[2])
        
        is_publishing = (frequency > 0)
        
        log_init = f"[Turn {self.current_turn}] Action selected: variant={variant_id}, channel={channel_id}, freq={frequency}"
        print(log_init)
        self._write_to_log(log_init)
...
Output only the CTI report text."""
            ...
            log_cpa = f"  >> Real CTI Context: {real_cti_text}\n  >> Generated Fake CTI: {self.fake_cti_text}"
            print(log_cpa)
            self._write_to_log(log_cpa)
            ...
            try:
                stealth_score = float(query_qwen(self.model, self.tokenizer, eval_prompt, max_new_tokens=10))
                self.stealth_scores.append(stealth_score)
            except Exception:
...
Outline your next plan step and tools you want to run. Output only your plan."""
            ...
            log_victim = f"  >> Victim PentestGPT V2 Plan: {plan_output}"
            print(log_victim)
            self._write_to_log(log_victim)
...
def main():
    ...
    try:
        with open("rq2_report.json", "w", encoding="utf-8") as f:
            json.dump(report, f, indent=2, ensure_ascii=False)
        print("Experiment report successfully saved to rq2_report.json")
    except Exception as e:
